...
function smoothDampFloat(
  ...
  velRef: { v: number }, smoothTime: number,
  maxSpeed: number, deltaTime: number
...
export default function GridScan({
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (enableGyro && typeof window !== 'undefined' && (window as unknown as { DeviceOrientationEvent?: { requestPermission?: () => Promise<void> } }).DeviceOrientationEvent?.requestPermission) {
        try { await (window as unknown as { DeviceOrientationEvent: { requestPermission: () => Promise<void> } }).DeviceOrientationEvent.requestPermission() } catch { /* noop */ }
      }
    }
    const onLeave = () => {
      if (uiFaceActive) return
  ...
  }, [sensitivity, lineThickness, linesColor, scanColor, scanOpacity, gridScale, lineStyle, lineJitter, scanDirection, enablePost, noiseIntensity, bloomIntensity, scanGlow, scanSoftness, scanPhaseTaper, scanDuration, scanDelay, bloomThreshold, bloomSmoothing, chromaticAberration])
      ...
      const detect = async (ts: number) => {
        ...
        try {
          ...
          if (res?.detection) {
            ...
            medianPush(bufX.current, (cx / vw) * 2 - 1, 5)
            medianPush(bufY.current, (cy / vh) * 2 - 1, 5)
            ...
            const dL = dist2(tip, jaw[3] || jaw[2]), dR = dist2(tip, jaw[13] || jaw[14])
            const eyeDist = Math.hypot(rc.x - lc.x, rc.y - lc.y) + 1e-6
            medianPush(bufYaw.current, Math.tanh(THREE.MathUtils.clamp((dR - dL) / (eyeDist * 1.6), -1, 1)), 5)
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-full overflow-hidden ${className ?? ''}`} style={style}>
      {showPreview && (
        <div className="right-3 bottom-3 absolute bg-black shadow-[0_4px_16px_rgba(0,0,0,0.4)] border border-white/25 rounded-lg w-[220px] h-[132px] overflow-hidden text-[12px] text-white pointer-events-none">
